...
abstract class REST_Controller extends CI_Controller
{
	...
	public function __construct()
	{
		...
		{
			$this->response(array('status' => false, 'error' => 'Only AJAX requests are accepted.', 'errorCode' => '505'), 505);
		}
	}
	...
	public function _remap($object_called, $arguments)
	{
		...
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			...
			$this->response(array('status' => false, 'error' => 'Invalid API Key.', 'errorCode' => '403'), 403);
		}

		// Sure it exists, but can they do anything with it?
		if ( ! method_exists($this, $controller_method))
		{
			$this->response(array('status' => false, 'error' => 'Unknown method.', 'errorCode' => '404'), 404);
		}
		...
		if (config_item('rest_enable_keys') AND !empty($this->rest->key))
		{
			...
			{
				$this->response(array('status' => false, 'error' => 'This API key has reached the hourly limit for this method.', 'errorCode' => '401'), 401);
			}
			...
			$authorized OR $this->response(array('status' => false, 'error' => 'This API key does not have enough permissions.', 'errorCode' => '401'), 401);
		}
	...
	protected function _force_login($nonce = '')
	{
		...
		$this->response(array('status' => false, 'error' => 'Not authorized', 'errorCode' => '401'), 401);
	}
